...
export default function CompnyRuninBeltCs() {
    ...
    const [sliderRef] = useKeenSlider<HTMLDivElement>({
        ...
        breakpoints: {
            ...
            "(max-width: 480px)": { slides: { perView: 2, spacing: 0 } },
        },
        ...
        <section className="py-12 bg-white text-center">
            ...
            <div ref={sliderRef} className="keen-slider max-w-7xl mx-auto">
                {brands.map((src, index) => (
                    <div
                        key={index}
                        className="keen-slider__slide flex items-center justify-center relative h-20 w-40"
                    >
                        <Image
                            src={src}
                            alt={`Brand ${index + 1}`}
                            ...
                            placeholder="blur"
                            blurDataURL={src}
                            className="object-contain grayscale hover:grayscale-0 transition-all duration-400"
                        />
                    </div>
                ))}
            </div>
